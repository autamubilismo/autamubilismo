...
const BackButton = ({ to = "/", theme }) => {
  ...
  const resolvedTheme = theme || contextTheme || 'light';
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden mix-blend-screen">
         <div className={`absolute top-[-10%] left-[-5%] w-[600px] h-[600px] rounded-full blur-[120px] opacity-20 ${isLight ? 'bg-cottage-rosy' : 'bg-[#bd00ff]/30 animate-pulse'}`} />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex-1 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
            {/* Ícone de Destaque */}
            <div className={`p-8 rounded-full mb-8 relative group transition-transform hover:scale-110 duration-500 ${isLight ? 'bg-cottage-rosy text-cottage-beige ring-8 ring-cottage-beige' : 'bg-[#1a1a20] border border-cyan-500/30 text-cyan-400 shadow-[0_0_40px_rgba(0,255,242,0.3)]'}`}>
                ...
                <Sparkles className={`absolute -top-2 -right-2 w-8 h-8 animate-bounce ${isLight ? 'text-cottage-rosy' : 'text-fuchsia-500'}`} />
            </div>
            ...
            <Link 
                ...
                className={`group px-10 py-4 rounded-full font-black uppercase tracking-[0.2em] transition-all duration-300 hover:scale-105 flex items-center gap-3 shadow-xl ${
                    isLight 
                    ? 'bg-gray-900 text-white hover:bg-cottage-rosy hover:shadow-cottage-rosy' 
