...
const GlossaryPage = () => {
  ...
  const cardBg = isLight 
    ? 'bg-white/70 backdrop-blur-md border border-cottage-rosy hover:border-cottage-rosy shadow-[0_10px_30px_rgba(255,182,193,0.3)]' 
  ...
  const accentText = isLight ? 'text-cottage-rosy' : 'text-fuchsia-400 drop-shadow-[0_0_5px_rgba(232,121,249,0.8)]';
  ...
  const lightPattern = {
    ...
    backgroundImage: `
      ...
      conic-gradient(from 0deg at 50% 50%, rgba(211, 150, 140, 0.1) 0deg, transparent 60deg, rgba(211, 150, 140, 0.08) 120deg, transparent 180deg, rgba(211, 150, 140, 0.1) 240deg, transparent 300deg, rgba(211, 150, 140, 0.08) 360deg)
    `,
      ...
      <div className={`min-h-screen p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans`} style={isLight ? lightPattern : darkPattern}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full pb-12">
          ...
          <div className="mb-12 text-center md:text-left relative">
            <span className={`inline-block px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-4 border ${isLight ? 'bg-white/50 border-cottage-rosy text-cottage-rosy' : 'bg-[#bd00ff]/10 border-[#bd00ff]/30 text-fuchsia-400 shadow-[0_0_10px_rgba(189,0,255,0.3)]'}`}>
               Glossário
            </span>
          ...
          {/* Busca Global */}
          <div className={`relative mb-12 p-1 rounded-[2rem] bg-gradient-to-r ${isLight ? 'from-cottage-rosy via-white to-cottage-rosy' : 'from-cyan-500 via-fuchsia-500 to-cyan-500'} shadow-lg`}>
             <div className={`relative ${isLight ? 'bg-white' : 'bg-[#09090c]'} p-4 rounded-[1.8rem] flex items-center gap-4 transition-all focus-within:shadow-inner`}>
                <Search className={`w-6 h-6 ${isLight ? 'text-cottage-rosy' : 'text-cyan-400'}`} />
                <input 
                  type="text" 
                  placeholder="Buscar categorias..." 
                  className={`bg-transparent w-full outline-none font-bold text-lg ${isLight ? 'text-gray-800 placeholder-cottage-rosy' : 'text-white placeholder-gray-600'}`}
                  value={searchTerm}
              ...
              return (
                ...
                >
                  <div className="mb-8 relative">
                     ...
                     <div className={`absolute -inset-4 rounded-full blur-2xl opacity-0 group-hover:opacity-40 transition-opacity duration-500 ${isLight ? 'bg-cottage-rosy' : 'bg-fuchsia-600'}`} />
                     
                     <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-110 shadow-lg relative z-10 ${isLight ? 'bg-white text-cottage-rosy ring-1 ring-cottage-rosy' : 'bg-[#1a1a20] text-cyan-400 border border-cyan-500/30 shadow-[0_0_15px_cyan]'}`}>
                        <Icon size={32} strokeWidth={1.5} />
                     </div>
                     
                     <h3 className={`text-3xl font-black uppercase mb-3 tracking-tight ${
                       isLight
                         ? 'group-hover:text-cottage-rosy'
                         : 'group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r from-cyan-400 to-fuchsia-500'
                     } transition-all ${textMain}`}>
                        {set.title}
                     </h3>
                  ...
                  <div className={`flex items-center justify-between border-t pt-6 ${isLight ? 'border-cottage-rosy' : 'border-white/10'}`}>
                     <span className={`text-[10px] font-black uppercase tracking-[0.25em] opacity-60 ${textMain}`}>
                       {set.items.length} Termos
                     </span>
                     <div className={`p-3 rounded-full transition-all duration-300 transform group-hover:translate-x-2 ${isLight ? 'bg-cottage-beige text-cottage-rosy' : 'bg-white/5 text-cyan-400 group-hover:bg-cyan-500/20 group-hover:shadow-[0_0_10px_cyan]'}`}>
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 animate-in fade-in slide-in-from-right duration-500 font-sans`} style={isLight ? lightPattern : darkPattern}>
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden mix-blend-screen">
         <div className={`absolute top-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full blur-[100px] opacity-20 ${isLight ? 'bg-cottage-rosy' : 'bg-cyan-900/40 animate-pulse'}`} />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto w-full pb-12">
        <button 
          ...
          className={`group mb-8 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all px-5 py-2.5 rounded-full ${
            isLight 
              ? 'bg-white/50 text-gray-500 hover:text-cottage-rosy hover:bg-cottage-beige border border-cottage-rosy' 
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6 mb-12">
           <div className={`w-20 h-20 rounded-[1.5rem] flex items-center justify-center shadow-xl ${isLight ? 'bg-white text-cottage-rosy ring-4 ring-cottage-beige' : 'bg-[#1a1a20] text-fuchsia-400 border border-fuchsia-500/30 shadow-[0_0_20px_rgba(232,121,249,0.3)]'}`}>
              {selectedSet.icon && <selectedSet.icon size={40} strokeWidth={1.5} />}
           </div>
           <div>
              <div className={`flex items-center gap-2 mb-2 text-[10px] font-black uppercase tracking-[0.25em] ${isLight ? 'text-cottage-rosy' : 'text-cyan-400'}`}>
        ...
        {/* Busca Interna */}
        <div className={`relative mb-10 p-1 rounded-[2rem] bg-gradient-to-r ${isLight ? 'from-cottage-rosy to-white' : 'from-[#1a1a20] to-[#0a0a0c] border border-white/10'}`}>
                 ...
                 return (
                     <div 
                       ...
                       className={`group cursor-pointer rounded-[1.5rem] overflow-hidden transition-all duration-500 border ${
                         isLight 
                           ? `bg-white/60 hover:bg-white border-cottage-beige hover:border-cottage-rosy ${isExpanded ? 'shadow-lg ring-1 ring-cottage-rosy bg-white' : 'shadow-sm'}` 
                     ...
                     >
                         <div className="p-6 md:p-7 flex items-center justify-between">
                            ...
                            <div className={`p-2 rounded-full transition-all duration-300 ${isExpanded ? (isLight ? 'bg-cottage-rosy text-cottage-beige rotate-180' : 'bg-cyan-900/30 text-cyan-400 rotate-180') : (isLight ? 'bg-gray-50 text-gray-400 group-hover:bg-cottage-beige group-hover:text-cottage-rosy' : 'bg-white/5 text-gray-500 group-hover:text-cyan-200')}`}>
                               <ChevronDown size={20} />
                            </div>
                         </div>
                         ...
                         <div className={`px-6 md:px-7 overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-96 pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className={`h-px w-full mb-6 ${isLight ? 'bg-cottage-rosy' : 'bg-white/5'}`} />
