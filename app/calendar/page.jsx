  ...
  return (
    <Link 
      ...
      className={`inline-flex items-center gap-2 mb-6 text-sm font-bold uppercase tracking-wider transition-all hover:-translate-x-1 duration-200 ${
        isLight 
          ? 'text-gray-500 hover:text-cottage-rosy' 
    ...
    <div className="mb-10 relative z-20 text-center md:text-left">
      {badge && (
        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 transition-all hover:scale-105 cursor-default ${
          isLight 
            ? 'bg-white text-cottage-rosy shadow-sm border border-cottage-rosy' 
...
const RaceCard = ({ race, isExpanded, onToggle, theme }) => {
  ...
  return (
    <div className={`relative rounded-[2rem] overflow-hidden transition-all duration-300 group ${
      isLight 
        ? 'bg-white/80 backdrop-blur-md border border-white shadow-[0_4px_20px_rgba(211,150,140,0.12)] hover:shadow-[0_8px_30px_rgba(211,150,140,0.22)] hover:-translate-y-1' 
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <div className="flex flex-col items-center justify-center min-w-[3.5rem]">
             ...
             <span className={`text-3xl md:text-4xl font-black italic ${
               isLight ? 'text-gray-200 group-hover:text-cottage-rosy transition-colors' : 'text-white/10 group-hover:text-cyan-400 transition-colors'
             }`}>
               {race.round}
             </span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className={`flex flex-wrap items-center gap-4 text-xs md:text-sm font-medium ${
              ...
              <div className="flex items-center gap-1.5">
                <MapPin size={14} className={isLight ? 'text-cottage-rosy' : 'text-fuchsia-400'} />
                {race.location}
              </div>
              <div className={`w-1 h-1 rounded-full ${isLight ? 'bg-gray-300' : 'bg-white/20'}`} />
              <div className="flex items-center gap-1.5">
                <CalendarIcon size={14} className={isLight ? 'text-cottage-rosy' : 'text-cyan-400'} />
          ...
          <div className={`p-2.5 rounded-full transition-all duration-300 ${
            isExpanded 
              ? `rotate-180 ${isLight ? 'bg-cottage-rosy text-cottage-beige' : 'bg-fuchsia-500 text-white shadow-[0_0_10px_rgba(255,0,255,0.5)]'}` 
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
        ...
        <div className={`p-6 pt-0 border-t mx-6 mb-4 ${
          isLight ? 'border-cottage-beige' : 'border-white/5'
        }`}>
            ...
            {race.sessions && race.sessions.map((session, idx) => {
              ...
              return (
                <div key={idx} className={`p-3 rounded-2xl flex flex-col justify-between h-full transition-all hover:scale-105 ${
                  isMainEvent
                    ? (isLight ? 'bg-gradient-to-br from-cottage-rosy to-cottage-rosy text-white shadow-md' : 'bg-gradient-to-br from-cyan-600/20 to-fuchsia-600/20 border border-fuchsia-500/40 shadow-[0_0_15px_rgba(255,0,255,0.1)]')
                  ...
                  {session.time && (
                    <div className={`mt-3 flex items-center gap-1.5 text-xs font-mono font-medium ${
                      isMainEvent 
                        ...
                        : (isLight ? 'text-cottage-rosy' : 'text-fuchsia-300')
                    }`}>
                      <Clock size={10} />
                      {session.time}
                    </div>
    ...
    <PageContainer theme={resolvedTheme}>
      ...
      <div className="mb-12 relative z-20">
        <h2 className={`text-xl font-black uppercase tracking-wider mb-6 flex items-center gap-3 ${
          ...
          <div className={`p-1.5 rounded-lg ${isLight ? 'bg-cottage-rosy text-cottage-beige' : 'bg-cyan-900/30 text-cyan-400 shadow-[0_0_10px_rgba(0,255,255,0.4)]'}`}>
             <Zap size={16} />
          </div>
          Pré-Temporada
        </h2>
            ...
            >
              <div>
                ...
                <p className={`text-sm font-medium mt-1 ${
                  ...
                  <span className={isLight ? 'text-cottage-rosy' : 'text-cyan-400'}>{test.circuitLabel}</span>
                  <span className="mx-2 opacity-30">|</span>
                  {formatPreseasonDate(test.startDate, test.endDate)}
                </p>
              </div>
              <div className={`p-3 rounded-full ${isLight ? 'bg-cottage-beige text-cottage-rosy' : 'bg-white/5 text-white/20'}`}>
                 <Clock size={24} />
              </div>
            </div>
          ))}
      ...
      <div className="relative z-20">
        <div className="flex items-end justify-between mb-8">
            <h2 className={`text-xl font-black uppercase tracking-wider flex items-center gap-3 ${
              ...
              <div className={`p-1.5 rounded-lg ${isLight ? 'bg-cottage-rosy text-cottage-beige' : 'bg-fuchsia-900/30 text-fuchsia-400 shadow-[0_0_10px_rgba(255,0,255,0.4)]'}`}>
      ...
      <div className="mt-16 pb-8 relative z-20">
        <Link
          ...
          className={`block p-8 rounded-[2.5rem] text-center transition-all duration-300 hover:scale-[1.02] ${
            isLight
              ? 'bg-gradient-to-r from-cottage-rosy to-cottage-rosy hover:shadow-lg hover:shadow-cottage-rosy'
        ...
        >
          <div className="flex flex-col items-center justify-center gap-2">
            <div className={`p-3 rounded-full mb-2 ${isLight ? 'bg-white text-cottage-rosy' : 'bg-white/10 text-cyan-300'}`}>
