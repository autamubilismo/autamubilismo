...
const NewsDetail = ({ theme = "light" }) => {
  ...
  useEffect(() => {
    if (!slug) {
        setPost({
            ...
            excerpt: "Uma performance de gala sob as luzes de Marina Bay reduz a diferença para Verstappen.",
            category: "Corrida",
            author: "Tamu",
  ...
  // ===== SEO DATA =====
  const siteUrl = 'https://autamubilismo.com'; // ⚠️ MUDE AQUI
  const newsUrl = `${siteUrl}/news/${slug || ''}`;
  const ogImage = post?.image 
    ? `${post.image}?w=1200&h=630&fit=crop&auto=format`
    : `${siteUrl}/og-default.pbg`;
  const ogDescription = post?.excerpt || (post ? `Leia ${post.title}` : '');
  const ogTitle = post ? `${post.title} | Autamubilismo` : 'Autamubilismo';

  if (loading) {
    return (
      <>
        <Helmet>
          <title>Carregando... | Autamubilismo</title>
        </Helmet>
  ...
  if (!post) {
    return (
      <>
        <Helmet>
          <title>Notícia não encontrada | Autamubilismo</title>
          <meta name="description" content="A notícia que você procura não foi encontrada." />
        </Helmet>
