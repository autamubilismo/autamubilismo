...
const GlossaryPage = ({ theme }) => {
  ...
  const cardBg = isLight ? 'bg-white border-2 border-transparent hover:border-gray-200 shadow-lg' : 'bg-[#1a1a20] border border-[#333] hover:border-[#caa5d8]/50';
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right duration-500 pb-12">
      <button 
        ...
        className={`group mb-8 inline-flex items-center gap-2 text-sm font-bold transition-colors ${isLight ? 'text-gray-500 hover:text-[#D8C4F0]' : 'text-gray-400 hover:text-[#caa5d8]'}`}
      >
            ...
            filteredTerms.map((item) => {
                ...
                return (
                    <div 
                      ...
                      className={`group cursor-pointer rounded-3xl overflow-hidden transition-all border ${isLight ? 'bg-white border-transparent hover:border-gray-200 shadow-sm' : `bg-[#1a1a20] ${isExpanded ? 'border-[#caa5d8]' : 'border-[#333] hover:border-[#caa5d8]/50'}`}`}
                    >
                        <div className="p-6 flex items-center justify-between">
                            <h3 className={`text-lg font-bold ${isExpanded ? (isLight ? 'text-purple-600' : 'text-[#caa5d8]') : textMain}`}>{item.term}</h3>
