...
const CircuitsPage = ({ theme }) => {
  ...
  const cardBg = isLight 
    ...
    : 'bg-[#121217] border border-[#333] hover:border-[#fe88dd]/50 shadow-lg';
  
  const accentText = isLight ? 'text-gray-800' : 'text-white';
  const subText = isLight ? 'text-gray-500' : 'text-gray-400';
  const iconColor = isLight ? 'text-[#F7B8C8]' : 'text-[#fe88dd]';
        ...
        {CIRCUITS_DATA.map((circuit) => (
          ...
          >
            ...
            <div className="p-6 relative">
                {/* Badge da Data */}
                <div className={`absolute -top-5 left-6 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest shadow-lg flex items-center gap-2 ${isLight ? 'bg-white text-gray-800 border border-gray-100' : 'bg-[#fe88dd] text-white'}`}>
