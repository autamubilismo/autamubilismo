      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 xl:col-span-4 order-2 lg:order-1">
           ...
           >
              ...
              <span className="absolute -bottom-10 -right-10 text-[15rem] font-black text-white opacity-20 select-none leading-none">{driver.number}</span>
              <img 
                src={fullBodyUrl} 
                alt={driver.name} 
                ...
                onError={(e) => { e.target.src = driver.image; e.target.className = "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full object-cover border-4 border-white shadow-xl"; }}
              />
              ...
              <div className="absolute bottom-8 left-0 w-full text-center z-30 lg:hidden">
                  <h1 className="text-4xl font-black text-white uppercase tracking-tighter drop-shadow-lg">{driver.name}</h1>
              </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-7 xl:col-span-8 order-1 lg:order-2 flex flex-col gap-6">
           <div className="hidden lg:block mb-4">
              <div className="flex items-center gap-4 mb-2">
                 <span className="px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest text-white shadow-md" style={{ backgroundColor: driver.color }}>{driver.team}</span>
                 <span className={`text-xs font-bold uppercase tracking-widest ${textSub}`}>{driver.country}</span>
              </div>
              <h1 className={`text-6xl xl:text-8xl font-black uppercase tracking-tighter leading-[0.9] ${textColor}`}>{driver.name}</h1>
           </div>
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className={`p-5 rounded-3xl ${cardBg} flex flex-col justify-between min-h-[120px]`}>
                 ...
                 <div><span className={`block text-2xl font-black ${textColor}`}>{driver.stats?.podiums || '0'}</span><span className={`text-[10px] font-bold uppercase ${textSub}`}>Pódios</span></div>
              </div>
              ...
              <div className={`p-5 rounded-3xl ${cardBg} flex flex-col justify-between min-h-[120px]`}>
                 ...
                 <div>
                    ...
                    {/* Alterado de "Categoria" para "Experiência" */}
                    <span className={`text-[10px] font-bold uppercase ${textSub}`}>Experiência</span>
                 </div>
              </div>

              {/* PONTOS (MELHOR RESULTADO SE FOI SUBSTITUIDO) */}
              <div className={`p-5 rounded-3xl ${cardBg} flex flex-col justify-between min-h-[120px]`}>
                 <Star className={isLight ? 'text-purple-500' : 'text-[#fe88dd]'} />
                 <div><span className={`block text-2xl font-black ${textColor}`}>{driver.stats?.bestResult || 'N/A'}</span><span className={`text-[10px] font-bold uppercase ${textSub}`}>Melhor Res.</span></div>
              </div>
              ...
              <div className="p-5 rounded-3xl flex flex-col justify-between min-h-[120px] text-white" style={{ backgroundColor: driver.color }}>
                 ...
                 <div><span className="block text-4xl font-black">{driver.number}</span><span className="text-[10px] font-bold uppercase opacity-80">Número</span></div>
              </div>
           </div>

           <div className={`p-8 rounded-[2.5rem] ${cardBg}`}>
              <h3 className={`text-xl font-black uppercase tracking-wide mb-4 flex items-center gap-2 ${textColor}`}><LayoutGrid size={20} /> Bio</h3>
              <p className={`text-lg leading-relaxed ${isLight ? 'text-gray-600' : 'text-gray-300'}`}>{driver.bio || "Biografia ainda não disponível."}</p>
              <div className="mt-8 pt-6 border-t border-gray-200/10 flex justify-end">
                 <span className={`font-handwriting text-4xl opacity-60 ${textColor}`} style={{ fontFamily: 'cursive' }}>{driver.signature || driver.name}</span>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={`p-6 rounded-3xl ${cardBg}`}>
                 ...
                 <div className="flex flex-wrap gap-3">
                    {driver.vibes && driver.vibes.map((vibe, i) => {
                        ...
                        return (<div key={i} className={`flex items-center gap-2 px-4 py-2 rounded-full border ${isLight ? 'bg-gray-50 border-gray-200 text-gray-700' : 'bg-[#1a1a20] border-[#333] text-gray-200'}`}>{Icon && <Icon size={16} className={isLight ? 'text-gray-400' : 'text-[#fe88dd]'} />}<span className="text-xs font-bold uppercase">{vibe.label}</span></div>)
                    })}
                 </div>
              </div>
              <div className={`p-6 rounded-3xl relative overflow-hidden ${isLight ? 'bg-gray-900 text-white' : 'bg-[#fe88dd]/10 border border-[#fe88dd]/30 text-white'}`}>
                 ...
                 <p className="text-lg font-medium italic">"{driver.secretFact || "Nada declarado..."}"</p>
              </div>
           </div>
        </div>
      </div>
