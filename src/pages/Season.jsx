    ...
    <div className="animate-in fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className={`flex p-1 rounded-full ${isLight ? 'bg-gray-200' : 'bg-[#1a1a20] border border-[#333]'}`}>
           ...
           <button onClick={() => setYear('2026')} className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${year === '2026' ? (isLight ? 'bg-white text-black shadow-sm' : 'bg-[#fe88dd] text-white shadow-[0_0_10px_#fe88dd]') : (isLight ? 'text-gray-500 hover:text-black' : 'text-gray-500 hover:text-white')}`}>2026</button>
        </div>
      </div>
      ...
      {year === '2025' ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
           ...
           <Link to="/circuits" className={`p-6 rounded-3xl h-48 cursor-pointer flex flex-col justify-between ${isLight?'bg-[#D8C4F0] text-purple-900':'bg-[#121217] border border-[#333] hover:border-[#fe88dd]'}`}>
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isLight?'bg-white/50':'bg-[#fe88dd]/20'}`}><MapPin size={24} className={isLight?'text-purple-800':'text-[#fe88dd]'}/></div>
        ...
        <Link to="/regulation" className={`p-8 rounded-3xl flex items-center justify-between cursor-pointer group transition-all hover:scale-[1.01] ${isLight ? 'bg-gradient-to-r from-[#D8C4F0] to-[#CFF7E8] text-gray-800' : 'bg-gradient-to-r from-[#fe88dd]/20 to-[#00fff2]/20 border border-[#fe88dd]'}`}>
          <div className="flex items-center gap-6">
             <div className={`w-16 h-16 rounded-full flex items-center justify-center shadow-lg ${isLight ? 'bg-white text-purple-600' : 'bg-black border border-[#fe88dd] text-[#fe88dd]'}`}><Scale size={32} /></div>
