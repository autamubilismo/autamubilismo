...
const client = createClient({
  projectId: 'c7nvssn2', // Substitua pelo seu Project ID real
  dataset: 'production',
  useCdn: true,
...
const BentoCard = ({ children, theme, className, onClick }) => {
  ...
  return (
    ...
    >
      {/* Grid Overlay para Vaporwave no Dark Mode */}
      {!isLight && (
...
const News = ({ theme }) => {
  ...
  return (
    <div className={`animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 font-sans min-h-screen transition-colors overflow-x-hidden ${
      ...
      {/* Background Decorativo Vaporwave */}
      {!isLight && (
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-20 relative z-10">
        ...
        <header className="mb-20 relative">
          ...
          <p className={`max-w-xl text-lg md:text-xl font-medium leading-relaxed ${
            ...
            Mergulhe nas atualizações que definem o ritmo do campeonato. <br />
            <span className={`text-xs font-bold uppercase tracking-widest mt-2 block ${
              ...
              Onde a velocidade encontra a informação.
            </span>
          </p>
        </header>
        ...
        {loading && !news.length ? (
          <div className="flex flex-col items-center justify-center py-40 animate-pulse">
            <div className={`w-20 h-20 border-t-4 border-l-4 rounded-full animate-spin mb-8 ${
              isLight ? 'border-pink-500' : 'border-cyan-500 shadow-[0_0_20px_cyan]'
            }`} />
            <span className={`text-xs font-black uppercase tracking-[0.6em] ${
              isLight ? 'text-gray-400' : 'text-fuchsia-400'
            }`}>Sincronizando...</span>
          </div>
        ) : (
